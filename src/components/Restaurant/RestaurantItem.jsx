...
export function RestaurantItem({ name, logo, rating, rateCount, image, delivey, status, promotion, minimumOrder, timeEstimation, distance, previewButton, isSelected, onCardClik }) {
    return (
            <div className="relative border rounded-md border-gray-200" >
                <div className='absolute z-50 top-2 right-2 cursor-pointer' hidden={!onCardClik} onClick={onCardClik}><LuEdit size={20} /></div>
                <div onClick={previewButton}> 
                    ...
                    <div className="p-1 py-2 flex justify-between items-center">
                        ...
                        <div className="flex text-xs font-semibold mt-auto gap-1" hidden={delivey == null}>
                            {delivey} SAR <TbTruckDelivery className='text-lg' />
                        </div>
